...
function renderExamResult({pyScore, sqlScore, total, passed, pyCorrect, sqlCorrect}){
  ...
  if(passed){
    ...
    const scoreLine = `${pyScore}/50 Python · ${sqlScore}/50 SQL (${total}/100)`;
    document.getElementById('certBuilder').dataset.score = scoreLine;
...
function generateCertId(){
  const d = new Date();
  const datePart = d.getFullYear().toString() + String(d.getMonth()+1).padStart(2,'0') + String(d.getDate()).padStart(2,'0');
  const rand = Math.random().toString(36).slice(2, 8).toUpperCase();
  return `CCA-${datePart}-${rand}`;
}
function loadCert(){
...
function generateCertificate(){
  ...
  const scoreLine = document.getElementById('certBuilder').dataset.score || '';
  const cert = {
    name,
    id: generateCertId(),
...
function renderCertificate(cert){
  ...
  document.getElementById('certScoreOut').textContent = cert.score;
...
function downloadCertificatePNG(evt){
  ...
  html2canvas(el, { backgroundColor:'#0A0C14', scale:2 }).then(canvas=>{
    const cert = loadCert();
