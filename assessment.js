...
const PASS_MARK = 40;
const EXAM_DURATION_SECONDS = 100 * 60; // 100-minute time limit

/* Sample certificate shown by default (before a visitor generates their own),
   representing Ashwik Bire's own completed assessment. */
const DEFAULT_CERT = {
  name: 'Ashwik Bire',
  id: 'CERT-PYSQL-2026-000001',
  refId: 'REF-PYSQL-2026-000001',
...
let examIndex = 0;
let examAnswers = {};
let examTimeLeft = EXAM_DURATION_SECONDS;
...
function refreshExamIntroUI(){
  ...
  if(lockedNote) lockedNote.style.display = complete ? 'none' : 'block';
  if(outNote) outNote.style.display = (!hasPassed() && attemptsLeft <= 0) ? 'block' : 'none';
  if(startBtn) startBtn.disabled = !complete || (!hasPassed() && attemptsLeft <= 0);
  ...
  if(savedCert && hasPassed()){
    ...
    // No certificate generated yet on this browser — show the sample
    // certificate (Ashwik's own completed assessment) as a preview,
    // without affecting the exam-intro/attempts flow for this visitor.
    renderCertificate(DEFAULT_CERT, {scroll:false});
  }
}
...
function startExam(){
  ...
  if(ids.length === 0 || done !== ids.length){
    document.getElementById('examLockedNote').style.display = 'block';
    return;
  }
  if(!hasPassed() && getAttempts() >= MAX_ATTEMPTS){
    document.getElementById('examOutOfAttempts').style.display = 'block';
    return;
  }
  ...
  document.getElementById('certBuilder').style.display = 'none';
...
function updateExamTimerUI(){
  const out = document.getElementById('examTimerOut');
  const box = document.getElementById('examTimer');
  if(!out || !box) return;
  out.textContent = formatTimer(Math.max(0, examTimeLeft));
  box.classList.toggle('low', examTimeLeft <= 5*60);
}
...
function renderExamResult({pyScore, sqlScore, total, passed, pyCorrect, sqlCorrect}){
  ...
  box.innerHTML = `
    ...
    <p style="color:var(--ink-dim); margin:0 0 6px;">${passed
      ? 'Both sections cleared the 40-mark bar. You can now generate your certificate below.'
  ...
  if(passed){
    document.getElementById('certBuilder').style.display = 'block';
    const builder = document.getElementById('certBuilder');
    builder.dataset.pyScore = pyScore;
    builder.dataset.sqlScore = sqlScore;
    builder.dataset.total = total;
...
function generateCertId(){
  ...
  return `CERT-PYSQL-${datePart}-${rand}`;
}
function generateRefId(){
  const d = new Date();
  const datePart = d.getFullYear().toString();
  const rand = String(Math.floor(Math.random()*999999)+1).padStart(6,'0');
  return `REF-PYSQL-${datePart}-${rand}`;
}
function loadCert(){
...
function generateCertificate(){
  ...
  const name = nameInput.value.trim() || 'Ashwik Bire';
  const builder = document.getElementById('certBuilder');
  const pyScore = parseFloat(builder.dataset.pyScore || '0');
  const sqlScore = parseFloat(builder.dataset.sqlScore || '0');
  const total = parseFloat(builder.dataset.total || String(pyScore+sqlScore));
  const cert = {
    name,
    id: generateCertId(),
    refId: generateRefId(),
...
function renderCertificate(cert, opts){
  ...
  document.getElementById('certPyOut').textContent = `${cert.pyScore}/50`;
  document.getElementById('certSqlOut').textContent = `${cert.sqlScore}/50`;
  document.getElementById('certTotalOut').textContent = `${cert.total}/100`;
  document.getElementById('certBuilder').style.display = 'none';
